...
`cafe-os`는 프랜차이즈가 아닌 개인 카페, 소규모 카페, 동네 카페를 위한 주문·운영 시스템입니다.
비싼 POS나 키오스크 없이 노트북, 모니터, 주방 태블릿만으로 주문 접수부터 매출 분석까지 운영하는 것을 목표로 합니다.
...
- 손님 주문을 주방 화면으로 전송
- 주방 태블릿에서 접수/완료 처리
- 주문 모니터링 창에서 접수/완료 주문번호 표시
- 매일 주문 내역 자동 저장
- 일자별, 시간별, 메뉴별 매출 분석
- 주문 당시 온도와 날씨를 함께 저장해 날씨별 매출 분석

## 운영 흐름

1. 테이블마다 주문 QR 코드를 붙입니다.
2. 손님은 QR 코드로 주문 화면에 접속합니다.
3. 손님이 주문하면 주문 내역이 주방 태블릿으로 전송됩니다.
4. 주방에서 접수 버튼을 누르면 주문 모니터링 창의 접수 영역에 주문번호가 표시됩니다.
5. 주방에서 완료 버튼을 누르면 주문번호가 완료 영역으로 이동합니다.
6. 영업 종료 후 매출 분석 프로그램에서 일별 매출, 시간대별 주문, 메뉴별 판매량을 확인합니다.

## 결제와 쿠폰 확장

현재 버전은 결제 시스템을 붙이지 않은 주문·모니터링·분석 프로그램입니다.

추후 결제 기능을 붙이면 운영 흐름은 아래처럼 확장됩니다.

```text
QR 주문 -> 주문 내용 확인 -> 토스 결제 -> 주방 전송 -> 접수/완료 -> 매출 분석
```
